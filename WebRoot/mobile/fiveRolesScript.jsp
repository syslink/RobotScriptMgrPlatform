...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		function loadScript(){
			showProcess('加载中......');
			var type = $("#Ttype").val();
			$.post("<%=basePath%>/cgi/script/selectlist.api", {uid:uid,type:type,role:5,API_AUTH_KEY:'7215EE9C7D9DC229D2921A40E899EC5F'},
				 function(data){
					$("#sid").empty();
					var length = data.dataList.length;
					for(var i=0;i<length;i++){
		...
		function  doexecute(){
			...
			var roles = "{\""+$('#input-group0').find("span").text()+"\":\""+$('#input-group0').find("select").val()
				        ...
				        +"\",\""+$('#input-group3').find("span").text()+"\":\""+$('#input-group3').find("select").val()
				        +"\",\""+$('#input-group4').find("span").text()+"\":\""+$('#input-group4').find("select").val()+"\"}";
		    
			var receiver = $('#deviceId1').val() + "," + $('#deviceId2').val() + "," + $('#deviceId3').val() + "," + $('#deviceId4').val() + "," + $('#deviceId5').val();
		...
		{
			 $("#content").val(action);
			 $("#receiver").val($("#deviceId1").val()+","+$("#deviceId2").val()+","+$("#deviceId3").val()+","+$("#deviceId4").val()+","+$("#deviceId5").val());
    ...
    <div class="alert alert-success" role="alert"> 
	  ...
	  <div class="input-group" id="input-group3" style="margin-top: 20px;width: 100%;">
				<span class="input-group-addon" >角色</span>
										
				<select  id="deviceId4" class="form-control" style="display: inline; width: 100%;height:40px;">
				 </select>
	  </div>
	  
	  <div class="input-group" id="input-group4" style="margin-top: 20px;width: 100%;">
				<span class="input-group-addon" >角色</span>
										
				<select  id="deviceId5" class="form-control" style="display: inline; width: 100%;height:40px;">
				 </select>
	  </div>
	  </div>
	...
	<div class="btn-group" style="margin-top: 40px;">
		...
		<div style="padding:5px 10px;text-align: center;">
		     <a type="button" href="threeRolesScript.jsp" class="btn btn-success btn-lg" >
		                      三人剧本
		     </a>
		</div>
