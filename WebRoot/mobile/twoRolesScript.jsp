...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		function loadScript(){
			showProcess('加载中......');
			var type = $("#Ttype").val();
			$.post("<%=basePath%>/cgi/script/selectlist.api", {uid:uid,type:type,role:2,API_AUTH_KEY:'7215EE9C7D9DC229D2921A40E899EC5F'},
				 function(data){
					$("#sid").empty();
					var length = data.dataList.length;
					for(var i=0;i<length;i++){
		...
		function  doexecute(){
			...
			showProcess('发送中......');
			var roles = "{\""+$('#input-group0').find("span").text()+"\":\""+$('#input-group0').find("select").val()+"\",\""+$('#input-group1').find("span").text()+"\":\""+$('#input-group1').find("select").val()+"\"}";
		    
			var receiver = $('#deviceId1').val() + ","+$('#deviceId2').val();
		...
		{
			 $("#content").val(action);
			 $("#receiver").val($("#deviceId1").val()+","+$("#deviceId2").val());
