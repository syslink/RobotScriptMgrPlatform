...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		$(function () {
			...
			loadRobots();
			$.post("<%=basePath%>/cgi/script/selectlist.api", {uid:uid,role:1,API_AUTH_KEY:'7215EE9C7D9DC229D2921A40E899EC5F'},
					   function(data){
					      hideProcess();
		...
		function  loadScript(sid){
			$.post("<%=basePath%>/cgi/script/get.api", {sid:sid,API_AUTH_KEY:'7215EE9C7D9DC229D2921A40E899EC5F'},
					function(data){
			  	        hideProcess();
			  	        document.cookie = sid + "=" + data.data;
			});
		...
		function  doexecute(){
			showProcess('发送中......');
			$.post("<%=basePath%>/cgi/script/execute.api", {sid:$('#sid').val(),receiver:$('#deviceId').val(),API_AUTH_KEY:'7215EE9C7D9DC229D2921A40E899EC5F'},
					  function(data){
				        hideProcess();
			          }
			);
	...
	<body style="text-align: center;padding:20px;">
	 ...
	 <div style="padding:5px 10px;text-align: center;margin-top: 50px;">
						     <button type="button" class="btn btn-success btn-lg" onclick="doLocalExecute()"  style="width: auto;">
						      <span class="glyphicon glyphicon-ok-circle" style="top:2px;"></span>本地执行（跟机器人要在同一局域网）
						     </button>
	 </div>				
